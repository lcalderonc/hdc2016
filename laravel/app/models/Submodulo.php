...
class Submodulo extends \Eloquent
{
    ...
    public static function updatePrivilegios($idUsuario,$idSubmodulo,$permisos)
    {
        ...
        switch ($permisos) {
            case 'abc': 
                $query->update(
                    array('agregar' => 1, 'editar' => 1, 
                    'eliminar' => 1, 'usuario_updated_at' => Auth::user()->id,
            ...
            case 'ab':
                $query->update(
                    array('agregar' => 1, 'editar' => 1, 
                    'eliminar' => 0, 'usuario_updated_at' => Auth::user()->id,
                    'updated_at' => date('Y-m-d h:i:s', time()))
                );
                break;
            case 'ac':
                $query->update(
                    array('agregar' => 1, 'editar' => 0, 
                    'eliminar' => 1, 'usuario_updated_at' => Auth::user()->id,
                    'updated_at' => date('Y-m-d h:i:s', time()))
                );
                break;
