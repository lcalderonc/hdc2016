...
class GeoplanController extends \BaseController
{
    ...
    public function postTecnicocelula() 
    {
        if (Request::ajax()) {
            $celula = explode("_", Input::get('celula_id'));

            $visorgps = new Visorgps();
            $tecArray["names"] = $visorgps->getCelulaTecnico($celula[1]);

            return json_encode($tecArray);
        }
    }
