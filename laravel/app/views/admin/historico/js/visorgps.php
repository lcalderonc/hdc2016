...
//DropDown elements
    ...
    function doTecList(teclist, icons) {
        try {
            ...
            $.each(teclist, function() {
                ...
                var grupo_html = "<select class=\"slct_grupotec\" id=\"tec_" + carnet + "\" name=\"tec_" + carnet + "\" onchange=\"dataGrupoTecnico('tec_" + carnet + "', '" + this.celula_id + "', '" + this.id + "')\">";
                grupo_html += "<option value=\"0\">Seleccione</option>";
                ...
                htmlTecList += "<div class=\"tecRow grupo" + this.grupo + "\">"
                        + "<div>"
                        + "<span>" + n + ".</span>"
                        + "<span>" + tecnico + "</span>"
                        + "</div>"
            ...
            $(".slct_grupotec").multiselect({
                nonSelectedText: 'Ninguno',
                includeSelectAllOption: true,
                nSelectedText: ' Grupos',
                allSelectedText: 'Todos'
            });
            $(".slct_grupotec").multiselect('rebuild');
            $(".slct_grupotec").multiselect('refresh');
    ...
    function dataGrupoTecnico(id, celula_id, tecnico_id) {
        var grupos = $("#" + id).val();
        Visorgps.DoSaveGrupoTecnico(celula_id, tecnico_id, grupos);
    }
    ...
    function showGrupo() {
        var grupo = $("#slct_grupo_tec").val();

        if (Object.size(tecMark) > 0 && Object.size(tecActu) > 0) {
            if (grupo == '') {
                //Mostrar todo
                $.each(tecMark, function() {
                    this.setMap(objMap);
                ...
                $.each($("#tec-list div"), function() {
                    $(this).show();
                });
            } else {
                //Mostrar solo grupo tecnicos
                $.each(tecMark, function() {
                    if (this.grupo == grupo) {
                        this.setMap(objMap);
                    } else {
                ...
                //Bloques del panel de tecnicos
                $.each($("#tec-list div"), function() {
                    var cssClass = $(this).attr("class");

                    if (typeof cssClass !== 'undefined') {
                        var busca = cssClass.indexOf("tecRow");
                        if (busca >= 0) {
                            //console.log(this);
                            if (cssClass == 'tecRow grupo' + grupo) {
                                $(this).show();
                            } else {
                                $(this).hide();
                            }
